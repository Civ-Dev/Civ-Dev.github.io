<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Civilisations 2: The Rebirth</title>

<!-- Bootstrap -->
<link rel="stylesheet" href="../OneDrive/Documents/Github Website/Civ-Dev.github.io/css/bootstrap.css">

<!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
	<style type="text/css">
	
		.jumbotron{
			margin-top: 20px;
			   background-image: url("https://i.imgur.com/bOMulMU.jpg");
			background-size: cover;
			color: white
		}
	
	
	
	</style>
</head>
<body>
<section>
  <div class="jumbotron text-center">
    <div class="container">
      <div class="row">
        <div class="col-xs12">
			<div class="jumbotron">
  <h1>Civilisations 2: The Rebirth</h1>
  <p>A Modded Minecraft Modpack</p>
</div>
     
      </div>
    </div>
  </div>
	</div>	  
</section>
<section>
  <div class="container">
    <div class="row">
      <div class="col-xs-12 text-center">
        <h1>About us</h1>
        <p>Civilisations 2: The Rebirth is a Modded Minecraft Modpack which is made for experienced players. It has hundreds of mods and is filled with challenging custom recipes to make the pack unique and fun. The many mods allows the pack to be enjoyable for all players.</p>
      </div>
    </div>
  </div>
</section>
<section>
  <div class="container">
    <div class="row">
      <div class="col-xs-12 text-center">
        <!--<h1></h1>-->
        <p>The pack is currently in Alpha stage and is always being updated and improved. We are always looking for test gamers as we develop and make the pack better. Your feedback is always helpful and appreciated. For any feedback please post in the Issues section on Github.
          Feel free to join our Discord server for any queries or questions about the pack.</p>
      </div>
    </div>
  </div>
</section>
<section>
  <div class="container well">
    <div class="row">
      <div align="center" class="col-lg-4 col-md-4 col-sm-6">
        <div class="media">
          <div class="media-middle"> <a href="https://github.com/Civ-Dev"> <img height=100 class="media-object" src="https://i.imgur.com/NeYAHIc.png" alt="..."> </a> </div>
          <div class="media-body">
            <h2 align="center" class="media-heading">Github</h2>
Github is where the main developing of the game happens. You can post issues and even improvements through Github. </div>
        </div>
      </div>
      <div align="center" class="col-lg-4 col-md-4 col-sm-6">
        <div class="media">
          <div class="media-middle"> <a href="https://discord.gg/hunbYNV"> <img height=100 class="media-object" src="https://i.imgur.com/OrQ6ou2.png" alt="..."> </a> </div>
          <div class="media-body">
            <h2 align="center" class="media-heading">Discord</h2>
Discord is what we use to communicate and talk to players and where players can recieve updates and talk to staff. </div>
        </div>
      </div>
      <div align="center" class="col-lg-4 col-md-4 col-sm-6">
        <div class="media">
          <div class="media-middle"> <a href="https://minecraft.curseforge.com/projects/civ2"> <img height="100" class="media-object" src="https://i.imgur.com/xMKKGxR.png" alt="..."></a></div>
          <div class="media-body">
            <h2 align="center" class="media-heading">Curse</h2>
Curse is where you can access the game files and download the game and updates. The game is also available through the curse app. </div>
        </div>
      </div>
    </div>
  </div>
</section>
<hr>
<hr>
<footer class="text-center">
  <div class="container">
    <div class="row">
      <div class="col-xs-12">
        <p>Copyright © Civilisations 2: The Rebirth. All rights reserved.</p>
      </div>
    </div>
  </div>
</footer>
<!-- jQuery (necessary for Bootstrap's JavaScript plugins) --> 
<script src="../OneDrive/Documents/Github Website/Civ-Dev.github.io/js/jquery-1.11.3.min.js"></script> 
<!-- Include all compiled plugins (below), or include individual files as needed --> 
<script src="../OneDrive/Documents/Github Website/Civ-Dev.github.io/js/bootstrap.js"></script>
</body>
</html>
